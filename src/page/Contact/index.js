...
function Contact() {
  ...
  useEffect(()=>{
    axios.get(URL_BACKEND + `/api/info-companies`).then((res)=>{
      const Data = res.data;
      console.log(Data);
        ...
        <div className="row block">
          <div className={`${style.WrapperInfoCompany} d-flex`}>
            ...
            <div className={`${style.ContainerImage}`}>
              <img className={`${style.ImageCompany}`} src="https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=580&q=80" alt="" />
            </div>{" "}
            <div className={`${style.ContainerInfoCompany}`}>
              ...
              <div>
                ...
                <div>
                  {" "}
                  <span className={`${style.MenuContactItem}`}>Địa chỉ: </span>{" "}
                  <span className={`${style.MenuContactDetail}`}>{AddressCompany}</span>{" "}
                </div>{" "}
                <div>
                  {" "}
                  <span className={`${style.MenuContactItem}`}>Số điện thoại: </span> 
                  <span className={`${style.MenuContactDetail}`}>{PhoneNumberCompany}</span>{" "}
                </div>{" "}
                <div>
                  {" "}
                  <span className={`${style.MenuContactItem}`}>Email: </span>
                   <span className={`${style.MenuContactDetail}`}>{EmailCompany}</span>{" "}
                </div>{" "}
              </div>{" "}
            </div>{" "}
          </div>
